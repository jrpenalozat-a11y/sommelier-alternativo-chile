...
import HojaParra from './components/HojaParra';

// Familia de apps (enlaces cruzados). Se excluye esta misma app.
const APPS_FAMILIA = [
  { nombre: 'Sommelier de Chile', emoji: '🍷', url: 'https://sommelier-chile.vercel.app/' },
...
function App() {
  ...
  const [ecoIdx, setEcoIdx] = React.useState(0);
  const [instalado, setInstalado] = React.useState(false);
  const promptInstal = React.useRef(null);
  const t = useT();

  // Rotación de frases ecológicas
  React.useEffect(() => {
    const id = setInterval(() => setEcoIdx((i) => i + 1), 6000);
  ...
  const frasesEco = t.frasesEco || [];
  const fraseEco = frasesEco.length ? frasesEco[ecoIdx % frasesEco.length] : '';

  // Instalar la PWA (usa el prompt nativo si está disponible; si no, instrucciones)
  ...
  // Compartir (Web Share API, con respaldo a copiar enlace)
  const compartir = async () => {
    const datos = {
      title: 'Sommelier Alternativo de Chile',
    ...
    <div className="app">
      <header className="cabecera">
        <div className="cabecera-marca">
          <span className="cabecera-copa" aria-hidden="true"><HojaParra size={38} /></span>
          <div>
      ...
      {fraseEco && <div className="eco-banner" key={ecoIdx}>{fraseEco}</div>}
